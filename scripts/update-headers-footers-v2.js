...
import { ThemeProvider } from "@/components/theme-provider"`;
    ...
    // Find the closing of the component - try different patterns
    let oldClosing = null;
    let newClosing = null;
    
    // Pattern 1: Standard closing with two closing braces
    const closingMatch1 = content.match(/\s*\}\s*\}\s*$/);
    if (closingMatch1) {
      oldClosing = closingMatch1[0];
      newClosing = `\n        </main>\n        <Footer />\n      </div>\n      <WhatsAppButton />\n    </ThemeProvider>\n  )\n}`;
    }
    
    // Pattern 2: Closing with just one brace (some pages)
    if (!oldClosing) {
      const closingMatch2 = content.match(/\s*\}\s*$/);
      if (closingMatch2) {
        oldClosing = closingMatch2[0];
        newClosing = `\n        </main>\n        <Footer />\n      </div>\n      <WhatsAppButton />\n    </ThemeProvider>\n  )\n}`;
      }
    }
    
    // Pattern 3: Look for the last closing brace and section
    if (!oldClosing) {
      const lines = content.split('\n');
      for (let i = lines.length - 1; i >= 0; i--) {
        const line = lines[i].trim();
        if (line === '}' || line === '} ' || line === '}') {
          // Found a closing brace, check if it's the component closing
          const beforeLines = lines.slice(0, i);
          const hasSection = beforeLines.some(l => l.includes('</section>'));
          if (hasSection) {
            oldClosing = line;
            newClosing = `\n        </main>\n        <Footer />\n      </div>\n      <WhatsAppButton />\n    </ThemeProvider>\n  )\n}`;
            break;
